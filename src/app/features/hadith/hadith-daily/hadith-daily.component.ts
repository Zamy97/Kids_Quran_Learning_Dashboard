  ...
  template: `
    @if (hadith()) {
      <div class="h-full flex flex-col min-h-0 overflow-hidden max-w-4xl mx-auto w-full px-2 py-2 md:px-4 md:py-3">
        <h2 class="text-lg md:text-2xl font-bold text-primary dark:text-white mb-2 flex-shrink-0">💎 Today's Hadith</h2>

        <div class="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl shadow-lg dark:shadow-gray-900/50 flex-1 min-h-0 overflow-hidden flex flex-col justify-center">
          <blockquote class="hadith-text text-gray-800 dark:text-gray-100 leading-relaxed italic border-l-4 border-primary pl-4 overflow-hidden">
            {{ hadith()!.text }}
          </blockquote>
          <p class="text-gray-500 dark:text-gray-400 mt-2 hadith-ref flex-shrink-0">— {{ hadith()!.reference }}</p>
        </div>

        <details class="bg-purple-50 dark:bg-gray-700 rounded-xl mb-2 border-l-4 border-purple-500 dark:border-gray-600 flex-shrink-0 overflow-hidden">
          <summary class="p-2 cursor-pointer font-semibold text-primary dark:text-white text-sm">📖 Story</summary>
          <p class="text-gray-700 dark:text-gray-200 leading-relaxed px-2 pb-2 hadith-story overflow-hidden line-clamp-4">{{ hadith()!.story }}</p>
        </details>
        ...
        <details class="bg-green-50 dark:bg-gray-700 rounded-xl border-l-4 border-primary dark:border-gray-600 flex-shrink-0 overflow-hidden">
          ...
          <p class="text-gray-700 dark:text-gray-200 leading-relaxed px-2 pb-2 hadith-lesson overflow-hidden line-clamp-3">{{ hadith()!.lesson }}</p>
        </details>
      </div>
    }
  `,
